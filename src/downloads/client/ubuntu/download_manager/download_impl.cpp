...
DownloadImpl::start() {
    if (_dbusInterface == nullptr) {
        emit Download::error(_lastError); // We're in this state due to a previous error
        Logger::log(Logger::Error, "Invalid dbus interface");
        return;
    }
...
DownloadImpl::pause() {
    if (_dbusInterface == nullptr) {
        emit Download::error(_lastError);
        Logger::log(Logger::Error, "Invalid dbus interface");
        return;
    }
