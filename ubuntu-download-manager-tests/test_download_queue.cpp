...
void TestDownloadQueue::init()
{
    ...
    _first = new FakeDownload(QUuid::createUuid(), "first-path", QUrl(), QVariantMap(), QHash<QString, QString>(), _networkInfo, _reqFactory);
    _firstAdaptor = new DownloadAdaptor(_first);
    _second = new FakeDownload(QUuid::createUuid(), "second-path", QUrl(), QVariantMap(), QHash<QString, QString>(), _networkInfo, _reqFactory);
