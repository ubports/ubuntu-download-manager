...
void
TestDownload::testStartNotConnected() {
    _networkInfo->setOnline(false);
    _reqFactory->record();
    ...
    // start the download and assert that no request are used
    download->start();
    download->startDownload();
...
void
TestDownload::testStartPauseNotConnected() {
    _networkInfo->setOnline(false);
    _reqFactory->record();
    ...
    // start and pause and assert that the signals is not raised
    download->start();
    download->startDownload();
    download->pause();
    download->pauseDownload();
...
void
TestDownload::testResumeNotConnected() {
    _networkInfo->setOnline(false);
    _reqFactory->record();
...
void
TestDownload::testReconnectAndDownloading() {
    // start the download with no connection and emit the signals,
    // then assert that the request is correctly created
    _networkInfo->setOnline(false);
    _reqFactory->record();
    ...
    _networkInfo->emitNetworkAccessibleChanged(QNetworkAccessManager::Accessible);
    calledMethods = _reqFactory->calledMethods();
    QCOMPARE(1, calledMethods.count());
}

void
TestDownload::testReconnectAndResumeDownloading() {
    // similar to the above but with a resumed download
    _networkInfo->setOnline(false);
    _reqFactory->record();
    ...
    // assert that the raw header is set with 0
    RequestWrapper* requestWrapper = reinterpret_cast<RequestWrapper*>(
        calledMethods[0].params().inParams()[0]);
    QNetworkRequest request = requestWrapper->request();
    // assert that the request has the correct headers set
    QVERIFY(request.hasRawHeader("Range"));
    QByteArray rangeHeaderValue = "bytes=0-";
    QCOMPARE(rangeHeaderValue, request.rawHeader("Range"));
}
