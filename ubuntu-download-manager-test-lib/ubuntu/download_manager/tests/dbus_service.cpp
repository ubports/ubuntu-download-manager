...
DBusService::generateConfigFile(QString& error) {
    ...
    if (!QFile::exists(inPath)) {
        qDebug() << "DBus configuration errors.";
...
DBusService::start() {
    ...
    qDebug() << "DBus address" << _dbusAddress;
...
DBusService::stop() {
    ...
    if (_daemonProcess != nullptr) {
        ...
        if (_oldDBusAddress.isEmpty()) {
            qDebug() << "Removing DBUS_SESSION_BUS_ADDRESS";
            unsetenv("DBUS_SESSION_BUS_ADDRESS");
        } else {
            qDebug() << "Setting the address to" << _oldDBusAddress;
